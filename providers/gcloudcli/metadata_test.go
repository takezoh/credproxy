...
func TestMetadataHandler_tokenEndpoint_principalEmailPath_returnsToken(t *testing.T) {
	...
	if resp.AccessToken != "test-principal-token" {
		t.Errorf("access_token = %q, want %q", resp.AccessToken, "test-principal-token")
	}
}

func TestMetadataHandler_tokenEndpoint_principalEmailPath_saMode(t *testing.T) {
	dir := t.TempDir()
	script := filepath.Join(dir, "gcloud")
	content := `#!/bin/sh
for arg in "$@"; do
  case "$arg" in
    --impersonate-service-account=*) echo "sa-email-path-token"; exit 0 ;;
  esac
done
echo "user-token"
`
	...
	if resp.AccessToken != "sa-email-path-token" {
		t.Errorf("access_token = %q, want sa-email-path-token", resp.AccessToken)
	}
}
