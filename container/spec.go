...
// The bridge listens on ListenAddr and forwards every connection to
// ContainerSocketPath (a unix socket accessible inside the container).
type BridgeSpec struct {
	// ListenAddr is the TCP address to listen on inside the container (e.g. "127.0.0.1:8181").
	ListenAddr string
	// ContainerSocketPath is the unix socket path inside the container.
